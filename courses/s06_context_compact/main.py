...
THRESHOLD = 10  # number of messages to keep in history
KEEP_RECENT = 3
...
def microp_compact(messages: list[MessageParam]) -> list[MessageParam]:
    tool_results = []
    for msg in messages:
        if msg["role"] == "user" and isinstance(msg["content"], list):
            for block in msg["content"]:
                if isinstance(block, dict) and block.get("type") == "tool_result":
                    if getattr(block, "content", "").startswith("COMPRESSED_TOOL_RESULT:"):
                        continue
    ...
    for block in to_clear:
        ...
        block["content"] = f"COMPRESSED_TOOL_RESULT: replaced original output, tool={tool_name}"
    return messages
...
def auto_compact(messages: list[MessageParam]) -> list[MessageParam]:
    ...
    with open(path, "a") as f:
        for msg in messages:
            f.write(json.dumps(msg, indent=2, default=str) + "\n")
    print(f"[Saved history to {path}]")
    conversation_text = json.dumps(messages, indent=2, default=str)[:80000]
    ...
    if isinstance(response.content, list):
        for block in response.content:
            if getattr(block, "type", None) == "text":
                summary = getattr(block, "text", None)
                break

    return [
        MessageParam(role="user", content=f"[Conversation compressed. History saved to {path}]\n\n{summary}"),
        MessageParam(role="assistant", content="Understood. I have the context from the summary. Continuing."),
    ]
