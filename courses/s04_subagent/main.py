...
def run_subagent(prompt: str) -> str:
    system = f"You are a coding subagent at {Path.cwd()}. Complete the given task, then summarize your findings."
    m = model
    tools = get_tools(is_subagent=True)

    summary = "(no summary)"
    messages: list[MessageParam] = [MessageParam(role="user", content=prompt)]  # fresh context
    for _ in range(30):
        ...
        if response.stop_reason != "tool_use":
            text = "".join(filter(None, (getattr(b, "text", None) for b in response.content)))
            if text:
                summary = text
            break
...
def get_tools(is_subagent: bool = False) -> list[ToolParam]:
    ...
    task_tool: ToolParam = {
        "name": "task",
        "description": "Spawn a subagent with a fresh context. It shares the filesystem but not conversation history.",
        "input_schema": {
            "type": "object",
            "properties": {
                "prompt": {"type": "string"},
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                if name == "task":
                    ...
                    print(f"\033[32m > Dispatching {desc} to subagent: {str(args['prompt'])[:50]}...\033[0m")
                    out = run_subagent(str(args["prompt"]))
...
def run() -> None:
    ...
    system = f"You are a coding agent at {Path.cwd()}. Use the task tool to delegate exploration or subtasks."
